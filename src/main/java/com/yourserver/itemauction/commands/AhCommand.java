...
public class AhCommand implements CommandExecutor {
    ...
    private void sendHelp(Player player) {
        ...
        player.sendMessage("§7 3. Нажми на слот §f\"Указать цену\"§7 (справа) - откроется каталог всех предметов");
        player.sendMessage("§7 4. Найди нужный предмет (можно листать страницы) и кликни по нему");
        player.sendMessage("§7 5. На следующем окне подбери количество кнопками -64/-10/-1/+1/+10/+64");
        player.sendMessage("§7    (этот предмет у тебя быть не обязан - это просто условие для покупателя)");
        player.sendMessage("§7 6. Нажми §f\"Готово\"§7, затем на зелёное стекло §f\"Подтвердить\"§7 - лот опубликован");
